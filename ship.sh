...

{
  ...
  printf "## Install\n\n"
  printf "See the README for full install instructions. In short: download the ZIP, unzip it, drag \`Virtual Overlay.app\` to \`/Applications\`, then right-click → Open on first launch.\n\n"
  printf "## SHA-256\n\n"
  printf "\`\`\`\n%s  %s\n\`\`\`\n\n" "${SHA256}" "$(basename "${ZIP_PATH}")"
  printf "## First launch\n\n"
  printf "macOS will show an unidentified developer warning because Virtual Overlay uses private CoreGraphics / SkyLight APIs and cannot be notarized. Right-click \`Virtual Overlay.app\` → Open → Open once. After that, double-click launches normally.\n\n"
  printf "## Requirements\n\n"
