...
### Multi-display watermark collision root cause (2026-05-10T20:30:47.936-04:00)
- Cristian's desktop report exposed an app-shell/renderer plumbing bug, not another CGS resolver bug: `SpaceFingerprinter.currentSnapshots()` already emits one `SpaceSnapshot` per `NSScreen`, and `JSONFileSpaceNameStore` already keys CGS matches by `(cgsSpaceID, displayUUID)`, but `Sources/App/main.swift` threw away every snapshot except `snapshots.first`.
- `OverlayController` did create one overlay window per screen, but it held one global `currentText` and rendered that same text into every managed window. That made the top display's only Space display the main display's first-space name even when the underlying identities were distinct.
- Fix shipped: App now resolves display names per `SpaceSnapshot` and sends `[OverlayContent]`; `OverlayController` tracks text per `screenID`; option-click rename carries the clicked display ID through `OptionClickRenameController`, so renaming a secondary display writes that display's identity.
- Keep the `spans-displays` diagnostic in the toolkit: `defaults read com.apple.spaces spans-displays` returns `1` when Mission Control is in one-Space-across-displays mode. If set, macOS is intentionally not exposing independent per-display Spaces; with separate Spaces enabled this code path now preserves independent overlay names.
